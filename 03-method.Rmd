...
\subsection{Gráfico de Dependência Parcial (\textit{Partial Dependence Plot - PDP})}

O gráfico de dependência parcial mostra um resumo da relação de uma ou duas variáveis explicativas do modelo e sua dependência com o predito, por meio do efeito marginal. Com ele podemos observar, por exemplo, se existe uma relação entre as variáveis e se ela é linear, quadrática ou mais complexa \cite{friedmanpdp}.
...
\noindent e $x_C$ o subconjunto complementar das variáveis utilizadas no ajuste do modelo 
...
Vemos também em \cite{ice} uma recomendação de utilizar um agrupamento das curvas ICE, ou seja, ao invés de considerar uma curva para cada indivíduo consideramos $k$ curvas dos indivíduos similarmente agrupados. Essa clusterização está disponível no pacote \emph{ICEbox}, implementado pelos autores do artigo. No presente trabalho, seguimos essa recomendação e utilizamos a visualização considerando clusters na análise dos gráficos ICE.
...
Em outras palavras, os gráficos de ALE calculam a média das alterações nas previsões e as acumulam nesse intervalo definido, que é menor que o intervalo de possíveis valores que a variável assume \cite{ale}. 
